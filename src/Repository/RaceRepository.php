...
class RaceRepository extends ServiceEntityRepository
{
    ...
    public function findByIdAndTeam($raceid, $teamid)
    {
        ...
        $query = $entityManager
            ->createQuery('SELECT r.id, r.title, r.description, r.logoPath FROM App\Entity\Race r LEFT JOIN r.registrations rs WHERE r.id = :raceid AND rs.team = :teamid');
    ...
    public function findAllWhereMember($userid)
    {
        ...
        $query = $entityManager
            ->createQuery('SELECT r.id, r.title, r.description, r.logoPath FROM App\Entity\Race r LEFT JOIN r.registrations rs LEFT JOIN rs.team rt LEFT JOIN rt.members sl WHERE sl.id = :userid ORDER BY r.startShowingPeaks DESC');
